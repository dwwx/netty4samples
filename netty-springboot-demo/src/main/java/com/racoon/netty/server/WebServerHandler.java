...
public class WebServerHandler extends ChannelInboundHandlerAdapter {
    //获取日志文件对象
    private Logger logger = LoggerFactory.getLogger(WebServerHandler.class);
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        logger.info("链接报告IP:{}", channel.localAddress().getHostName());
        logger.info("链接报告Port:{}", channel.localAddress().getPort());
        logger.info("链接报告完毕");
        //通知客户端链接建立成功
        String str = "通知客户端链接建立成功" + " " + new Date() + " " + channel.localAddress().getHostName() + "\r\n";
